...
void CPFA_controller::Returning() {
 //LOG<<"Returning..."<<endl;
	...
	if(IsInTheNest()) {
	    ...
	    if(!selectedTarget && updateFidelity && poissonCDF_sFollowRate > r2) {
	        SetIsHeadingToNest(false);
      ...
      if(!selectedTarget && SetTargetPheromone()) {
          isInformed = true;
          isUsingSiteFidelity = false;
          selectedTarget = true;
      }
...
void CPFA_controller::SampleVisitedLocation() {
    ...
    if(VisitSamplePeriodTicks == 0) {
        VisitSamplePeriodTicks = 25 * SimulationTicksPerSecond();
    }
    if(CPFA_state == SEARCHING && !IsHoldingFood()) {
        size_t now = SimulationTick();
        if(LastVisitSampleTick == 0 || now - LastVisitSampleTick >= VisitSamplePeriodTicks) {
            VisitedLocationQueue.push_back(GetPosition());
            while(VisitedLocationQueue.size() > MaxVisitedLocations) {
                VisitedLocationQueue.pop_front();
            }
            LastVisitSampleTick = now;
        }
    }
}

void CPFA_controller::UploadVisitedLocations() {
    if(LoopFunctions == NULL || VisitedLocationQueue.empty()) return;
    std::vector<argos::CVector2> points(VisitedLocationQueue.begin(), VisitedLocationQueue.end());
    LoopFunctions->RecordVisitedLocations(points);
    VisitedLocationQueue.clear();
}
		         ...
		         for(i = 0; i < LoopFunctions->FoodList.size(); i++) {
		          ...
		          // We found food! Calculate the nearby food density.
	        	             ...
	        	             j = i + 1;
                                     searchingTime+=SimulationTick()-startTime;
                                     startTime = SimulationTick();
                                     // AHCFA: do not regenerate a resource after pickup.
                                     // The final project evaluates collection from a fixed set of 256 resources.
                                     break;
			             } else {
...
void CPFA_controller::SetLocalResourceDensity() {
    ...
    updateFidelity = true; 
	    ...
	    if(LoopFunctions->UseAHCFA == 1) {
	        LoopFunctions->RecordResourceDiscovery(SiteFidelityPosition, ResourceDensity);
	    }
